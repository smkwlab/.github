...
## audit-org-settings.sh

Renovate と secret scanning の稼働は、**リポジトリ側のファイルと GitHub 上の設定が
揃って初めて成立します**。前者はバージョン管理下にありますが、後者はそうではありません。
そして欠けても何も言いません。設定ファイルは正しく、preset も解決でき、ただ job が
来ないだけなので、リポジトリ側からは何の異常も見えません。

二度見落としています。Renovate App の対象から 11 リポジトリが漏れて **4 か月**、
それが解消されたことに **26 日**（smkwlab/.github#198）。欠落だけでなく解消も
見落としている点が、この監査が必要な理由です。

```bash
# 乖離があれば非ゼロ終了（読み取りのみ）
scripts/audit-org-settings.sh
scripts/audit-org-settings.sh --quiet   # 一致した項目を出さない
```

### 起点は org の全リポジトリ

エコシステム単位で数えると、**どちらにも属さないものが構造的に見えません**。実際に
secret scanning の棚卸しを latex 側と DNS 側が別々に行い、両方とも自分の枠内しか
数えていませんでした。org 全体では public 31 本のうち有効が 3 本で、差分には
どちらの棚卸しにも出てこなかったリポジトリが並んでいました。

`search/code` は使いません。レート制限や索引の遅れで 0 件を返し、**それが「該当なし」と
区別できない**ためです。`orgs/{org}/repos` を paginate して実体を列挙します。

この監査を動かすトークンは org の全リポジトリを見られる必要があります。既存の 2 監査と
同じ App（`repository_selection=all`）を使うのはそのためで、ここが `selected` だと
**監査自身が探しているものと同じ盲点を持ちます**。

### 分類

| 分類 | 意味 | 対処 |
|---|---|---|
| `drift` | renovate 設定があるのに Dependency Dashboard が無い / public なのに secret scanning が無効 | App への追加、または設定の有効化 |
| `leak` | 学生リポジトリが renovate 設定を持っている | テンプレート側を直す。生成物にコピーされるため放置すると増え続ける |
| `unknown` | 権限やレスポンスの都合で確認できなかった | **「異常なし」ではない**。0 件として数えない |
| `info` | renovate 設定を持たない稼働中リポジトリ | 判断待ち。対象にすべきものが混ざっていないか見る |

`unknown` を分けているのは、`drift: 0` と「確認できなかった」を同じに扱わないためです。
この監査を作る過程で、出力ゼロを「該当なし」と読み違える失敗を 4 回踏んでいます。
...
この監査だけは org 全体を起点にするので、**報告に latex にも DNS にも属さない
リポジトリが混ざります**。担当を決めずに置くと、どちらも自分の枠外だと判断して
誰も見ない状態になります。secret scanning の棚卸しを両者が別々に行い、両方とも
自分の枠内しか数えていなかったのと同じ失敗の形です。

| 分類 | 担当 |
|---|---|
| `drift` / `leak` / `unknown` | latex 側 |
| `info` | 担当を置かない（報告のみ） |

`drift` と `unknown` は監査基盤そのものの問題で、この監査とスクリプトは latex 側が
持っています。`leak` はテンプレート由来で、テンプレートも latex 側です。どちらも
対象リポジトリがどのエコシステムにあるかで変わりません。

`info` に担当を置かないのは、**判断材料が分類の中に無い**からです。`atcoder-env` に
Renovate を入れるべきかは、どちらのエコシステムの知識でも決まりません。担当を置かない
と決めるのと、決めないまま放置するのは別です。前者なら、必要だと判断した人がその時に
拾えます。

これは latex 側が引き取ると宣言したもので、DNS 側と合意した内容ではありません。
`info` の扱いを含め、異議があれば Issue を立ててください。

### 週次監査

`.github/workflows/audit-org-settings.yml` が毎週月曜 11:00 JST に audit を回し、
乖離があれば本リポジトリに Issue を起票します（既存の報告があればコメントを追記）。
他の 2 監査と同じ月曜なので、報告 Issue が同時に立たないよう時刻をずらしています。
揃えば Issue は自動で閉じます。
